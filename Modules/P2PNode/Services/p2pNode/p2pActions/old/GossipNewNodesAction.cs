...
public class GossipNewNodesAction : INodeAction //INode = temporary off
{
    ...
    {        
        ...
        {
            await GossipToNewPeers(context, manager, stoppingToken);
        }
    }

    private async Task GossipToNewPeers(ContextInstance context, NetManager manager, CancellationToken stoppingToken)
    {
        var newPeers = context.NewPeers;
        var updatedPeers = context.NewPeers.Where(p => !string.IsNullOrEmpty(p.Value.HashKey));
        if (updatedPeers.Count() == 0) return;

        var gossipPeersCount = context.Config.Get<int>(P2PSettings.GossipCount_NAME, 3, P2PSettings.SECTION);

        var selectedPeers = context.NewPeers.Take(gossipPeersCount);

        await Gossip(selectedPeers.Select(p => p.Value).ToArray(), context, manager, stoppingToken);
        await GossipTrackers(selectedPeers.Select(p => p.Value).ToArray(), context, manager, stoppingToken);

        foreach (var peer in selectedPeers)
        {
            newPeers.Remove(peer.Key, out _);
        }
    }
    ...
    private async Task GossipTrackers(NodeWorking[] selectedPeers, ContextInstance context, NetManager manager, CancellationToken stoppingToken)
    {
        var knownTrackers = context.Trackers.Values;
        if (knownTrackers.Count == 0) return;

        var peerList = knownTrackers.Select(p => NodeUpdate.FromNode(p)).ToList();
